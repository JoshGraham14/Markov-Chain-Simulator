...
def fill_t_matrix(t_matrix, nodes):
    ...
    for i in range(len(t_matrix)):
        for j in range(len(t_matrix[i])):
            while(True):
                try:
                    t_matrix[i][j] = float(input("Value for position [" + str(i+1) + ", " + str(j+1) + "]: "))
                    break
                except ValueError:
...
def steady_state(t_matrix, nodes):
    ...
    try:
        ...
        return ["There is not a possible steady state for this Markov chain."]
...
def simulate_steps(t_matrix, p_list):
    ...
    for i in range(len(p_list)):
        init_pop += "Node " + str(i+1) + ": " + str(p_list[i]) + "   "
    print("You will be shown the current population after each step. Press enter to continue, or type 'q' to quit.")
    input("initial node populations => " + init_pop)
    # loops for however many steps are specified
    # tqdm in the for loop adds a progress bar
    counter = 0
    while(True):
        ...
        user_in = input("step " + str(counter+1) + " node populations =>  " + str(nodes_pop))
